export const blueIds = {
  'PayNote/Cancel Before Completion Requested':
    '4oyKGB49m8MuAFdxgHZauwhDFvdBgziHnb9CCGeKNqEc',
  'PayNote/Capture Declined': 'FzJqLm7Ek58LsVstJV2c37JfiMhRiWtjZNsiMz2ZfCYG',
  'PayNote/Capture Failed': 'FUh3TARSh4TjnWKAkM5ydjFWLWEmrFByKMBQzcgQfqRW',
  'PayNote/Capture Funds Requested':
    'DvxKVEFsDmgA1hcBDfh7t42NgTRLaxXjCrB48DufP3i3',
  'PayNote/Card Charge Completed':
    'Hrz9kzWXTXDfK2XEkRJtHqdKzHaQq919NcRL8QMAvEEQ',
  'PayNote/Card Charge Responded':
    'CFoaaYn29o7Mg7VaN7bkM87KrfE8fvnVYWkGXd6YnQ7S',
  'PayNote/Card Transaction Capture Lock Change Failed':
    '2LFkmyxDFSdNqM8mr6Md3LF54V15fFGL1s5fTnUfCpUH',
  'PayNote/Card Transaction Capture Lock Requested':
    'DhxGBjA6Gow9E6ZKZ49SdziihHZ4PeXxFNatSqmesKZu',
  'PayNote/Card Transaction Capture Locked':
    'EXoQHkYEDQdhGd3AeXBryzxmCTXsvTjXTfZmjwuqqbHt',
  'PayNote/Card Transaction Capture Unlock Requested':
    '531sxtH5rD9ter3EzwBTcn9GMdA7RAYFwmSQRzxCru8z',
  'PayNote/Card Transaction Capture Unlocked':
    'DiowRXdCBw83YCn5Pwcg2YABaVQZ1p4Wk1L9DJfajqp5',
  'PayNote/Card Transaction Details':
    'GZLRe2fEsvs1v7dVcg9kEnCrWEdM3cUZYhFH4XqN5jQT',
  'PayNote/Card Transaction Monitoring Request Rejected':
    'B2iherBskP5f4x7x31GahNmsJtyzian6ZN92b8kE1E5q',
  'PayNote/Card Transaction Monitoring Started':
    'GcKHzzeu5qehMo1JLGN7X5tSzrHM2iJoscN2qmkB5RPm',
  'PayNote/Card Transaction Monitoring Stopped':
    'BYdTyyLphWQNKo1GBcnE1jQuaPyXexNnfzkXhMiRqmUr',
  'PayNote/Card Transaction PayNote':
    'GDmDBkzVuvpkNZHrUgNmpijC9rfq8YQ8f29nHHH9aC2X',
  'PayNote/Card Transaction Report':
    '2ibvMNB7oxcpkYpxpag2HLC81sRs3PUBFtqjbqN7ET8X',
  'PayNote/Child PayNote Issuance Declined':
    'DFKVw43E36kimqj64FyiiVxE9yNuB22SETFx5M4WAi9m',
  'PayNote/Child PayNote Issued':
    'FAZCx2s5eq9zPV64LdHNFYbjjxD3ci1ZqyTcQk5WhXAs',
  'PayNote/Complete Payment Requested':
    '6yEDRKSGUFUhLMYf3anEytTjjV4PneHnzmqcwtFQ2m4T',
  'PayNote/Final Amount Resolution Rejected':
    'E6zFPqoEmEdP9xFa1A9FDSsDJDb7BaR7tXakVD7RN8oH',
  'PayNote/Final Amount Resolution Requested':
    '3RkEm1oNz9wYn7CEvReyLduk71eU5e8psAeGyLXwZxr6',
  'PayNote/Final Amount Resolved':
    '7oKW3Fozo1KUPgxo4PdF6jJJQ83z11mBbpZF2xCENGDX',
  'PayNote/Funds Captured': 'BJvjorbC5ed5KTV7SxoV3CvrJXjrFPcFxY9QT4jHBbXi',
  'PayNote/Funds Reserved': 'AopfdGqnwcxsw4mJzXbmjDMnASRtkce9BZB1n6QSRNXX',
  'PayNote/Funds Secured': 'DvxVaiLspGpmTY5SiZDb85sJLcvzBCoJCjCHSAmVFbGT',
  'PayNote/Funds Securing Declined':
    '8PufMmsuBgpX5YCpiS7XuiCFUWDan1fN8c3TLLwh57gL',
  'PayNote/Funds Securing Failed':
    '67BprL1WeSSpJDKawcnaNC6VF2vz68JQ5qrVfi37biBA',
  'PayNote/Issue Child PayNote Requested':
    '53Dir2sGy1NHuCQXF6suGoDMxYacNhbcy23AKD89SghD',
  'PayNote/Linked Card Charge and Capture Immediately Requested':
    '2LqGeXL7WWYQJMqviyseKCekpY3wYdB6K1f9sqakBUY7',
  'PayNote/Linked Card Charge Requested':
    'Aq4dNFtLHLWYfCS7aqAgWwGi48MgbQmp2EZJtDBN7FSg',
  'PayNote/Linked PayNote Start Failed':
    '8dggwonfALwrTSRhg8g8ncXFXierke2mogtEZQXHab64',
  'PayNote/Linked PayNote Start Responded':
    'BQioEtRPYv2fWVryRsSYQc1Vnp9eyX3CYDrNY1hEy1Ye',
  'PayNote/Linked PayNote Started':
    '6vnMMWuq6qJ1hxLqL1P2ckCqC9JtJF3QNW8s7rMTgZ4Q',
  'PayNote/Merchant To Customer PayNote':
    'GT1pxgc28u4M6dkNtKJJWbex7wAaJ66nAut6b9tj97t4',
  'PayNote/Payee Assignment Confirmed':
    '34v52X6nVj6muiD11W8nohLFn7DjT2RiaRYwjRNpq4v3',
  'PayNote/Payee Assignment Rejected':
    'CNFxs2PfxjDh7HNCaehyxNJ8zAdLbmgTcH12rU8VA7yi',
  'PayNote/Payee Assignment Requested':
    'BVLMcTzag3D2rvg8LoKQ3Htgoqsh77EAoiTLTxH5aVBE',
  'PayNote/Payment Cancellation Declined':
    '4witPJDjFKHNHyucepuDn9KXyUTTqZQTVsKHvcrVbGvY',
  'PayNote/Payment Cancellation Failed':
    'ExfGtqDcSr7RkvA7XzdV5B1MVc7CPacHpanjGBtGhjyk',
  'PayNote/Payment Cancelled Before Completion':
    '9hvBQjesbYZPkZSaikm5avp7sFjTHsjRkQS2gas1u49p',
  'PayNote/Payment Completed': '72eeCYvygiChLj529TP1HKKBaYyB5TBa15Y3cn3JGsak',
  'PayNote/Payment Completion Declined':
    '9AL8hghruGSH1ibFSYE8N7xoPLWLkktTCMFRkdoNsfTN',
  'PayNote/Payment Completion Failed':
    'An4JGf9zvSwEtYPJjZaWRnHaPXXvmyHg7oNLmJRKuxSV',
  'PayNote/Payment Completion Lock Change Failed':
    'DG24you4q9reWwc93JPhCK21WoAtP8w9Y6HThyWmL41e',
  'PayNote/Payment Completion Lock Requested':
    '2aevbSbcGTCXR2n3Kftbsoo9yV4jHw9MFpbsRUDrMBDL',
  'PayNote/Payment Completion Locked':
    'BKBtKSbxAxg1vyg2ZcghVANjaS4viMxUFY2un6TDPWbw',
  'PayNote/Payment Completion Unlock Requested':
    'BqygExH4phfPLuES2VLGpYL2gDenFFCcukPXzC1PmQtM',
  'PayNote/Payment Completion Unlocked':
    'BZxWkxpxVygutoo7KuBd7C63nZwYDuPXFhffXhZ319z9',
  'PayNote/Payment Mandate': 'BKKgKnHFhLmmUxq4WRvhV7HYZNu6ngDEce1LvHjZimJr',
  'PayNote/Payment Mandate Attached':
    '49TYrHpnk2gepJiGduJP3afrnT2DJ6kDxqF4Y9M4C4t7',
  'PayNote/Payment Mandate Attachment Failed':
    'GTwsVrbVb31sMub1vvU2KyY2nA8ekKWYDoqNAB1m4Vh2',
  'PayNote/Payment Mandate Spend Authorization Requested':
    '7EKvVzbT63C2taKWfLf9J2BiVL7BCL6Ld86tH8b9kmxF',
  'PayNote/Payment Mandate Spend Authorization Responded':
    '6UFd89oNox3PCpeVFaaCXwbGDp3k3Qc56e4DktnK3F8P',
  'PayNote/Payment Mandate Spend Settled':
    '6aPqmL9AQV31CCV576ZmEcUgysbdbQAfAig9J5VYeR6v',
  'PayNote/Payment Mandate Spend Settlement Responded':
    'CoGAMipt8rhmnkMgKsyZ5aendkHvFc4muC6pVgrUT1rJ',
  'PayNote/Payment Reversal Declined':
    'Fk96a7GFSo2c3w7xWzaqnYhXXhLAAxj9Dwao8Hg9sdX',
  'PayNote/Payment Reversal Failed':
    'FNZQJMETUdV57VFyWHGPdQVPSjWsvGj7u4n2oKYbgAaH',
  'PayNote/Payment Reversal Lock Change Failed':
    'DYUz7mcFpgQdRNwwsSSBdDe3CYtfRbAeAUiEv6tuLDhy',
  'PayNote/Payment Reversal Lock Requested':
    '2qrvwZRrYWZvZZncsQHNbcQRXQDhoFQPcUsVxoT34L69',
  'PayNote/Payment Reversal Locked':
    '5fKdfddLui1nNGHq2hKyEZm8pR6vaN7BRcuomWJPj82Z',
  'PayNote/Payment Reversal Unlock Requested':
    '2xFW6D91Vx628zDGqPwyEPQAdUnmvm9G8Eki5pMeHWi4',
  'PayNote/Payment Reversal Unlocked':
    'HPLY55rtyD7BVZaHWhB9iUP7AoFTykn6ZCF3K3BaBbVu',
  'PayNote/Payment Reversed After Completion':
    '81whmkSDanPULQUi4sMuVkxiWDLHb3VPC5PeLfJCXCGo',
  'PayNote/PayNote': 'GJkuHuw2e1cTzoS2A2nuzK9Hg64yedbFb6ayrcqz7u1d',
  'PayNote/PayNote Acceptance Requested':
    '3kyFUyupzb49ZoxVHnUhVe4XAbEN1Hpy8zN9Dx75GNyc',
  'PayNote/PayNote Accepted': 'CfSpcRXYk2qwu1vNs9LL8rycBsxzL2R4LGyDdrDzwCjh',
  'PayNote/PayNote Accepted By Client':
    '6VQC585V4SdpyA5hCeQQGM178bbeW3D2WcKt8ZCWK9s3',
  'PayNote/PayNote Approved': 'HQTUxErobqhSuhWo9DAC1WwaG9oYdjfmdKprGtV4TeEK',
  'PayNote/PayNote Cancellation Rejected':
    'GaYDPA7TTqWuoxioCYFPeyqomjH4g3YDtFxHv9yLRQ8A',
  'PayNote/PayNote Cancellation Requested':
    'DqiwzsNLbHCh6PaDF6wy6ZqBSF5JV5nAQSKFKTPRTbGB',
  'PayNote/PayNote Cancelled': '96buyUXwhkak8xKoCR5nAW9tMuwzkevJFdELVvwKxR6Y',
  'PayNote/PayNote Client Decision Discarded':
    'Da7ZSyWgvMyTfwDVhAgCkGf3H8dwHhouHsHgNzg3DZ2j',
  'PayNote/PayNote Delivery': '2eBsi15koVLa2rXUBiiNGduAQuGj4SzeRf9Xjr5YJaDT',
  'PayNote/PayNote Delivery Failed':
    'GtFG4Nt2fAamUZi9fSZNotab3BEUuv236LuPAcErVj5y',
  'PayNote/PayNote Rejected': 'AdKfkwRfzRUxUKSzhRfYANsaUBNnz4u6JFWR66qhzyZe',
  'PayNote/PayNote Rejected By Client':
    'EGRRGwNnReqfQQhKnML28DWz9MvvC3B5JgbBrCUxrZ3G',
  'PayNote/Reservation Declined':
    '4xS8bmZQBGPENmaPfsrtYguYfq4hTtaZAXrefdyFNkKq',
  'PayNote/Reservation Release Declined':
    '653sCbbRH3RiKhGjmVxh6wFVs4rn54wJRKDXRMKBZtjA',
  'PayNote/Reservation Release Requested':
    'GU8nkSnUuMs6632rHQyBndRtjDcMB9ZSbgwkGYcfGt97',
  'PayNote/Reservation Released':
    'CFqiZigjKE5JatANkaAkWw2NbgvEmb2BVEVPf3ckUrWg',
  'PayNote/Reserve Funds and Capture Immediately Requested':
    '3XstDYFkqsUP5PdM6Z6mwspPzgdQMFtUpNyMsKPK2o6N',
  'PayNote/Reserve Funds Requested':
    '3Y3TYmSfZMmPYKmF5i3eR8YcVPNP5Sic2bZN8xRnvMWm',
  'PayNote/Reverse After Completion Requested':
    'AG1PERDv5Vzxx3GTpeDKpEdMD3bPbMcijHndAPPdH8zc',
  'PayNote/Reverse Card Charge and Capture Immediately Requested':
    'F4gfZeY8P8dkfwNSbEfq7xQ8axiHdfqgzGQQr3HLDyzC',
  'PayNote/Reverse Card Charge Requested':
    'Fg5CEPi8Dbr1vBwGjzf8PbQNfuWfPP7HcLFChYwpRJq5',
  'PayNote/Secure Funds Requested':
    '94M8FmSvYDJYBZwDvCuVYRQmYxD1Ek6cQSTMKEYYXbY6',
  'PayNote/Settlement Amount Rejected':
    '3b3ePGPg5GzS6KYfqoDfgjWbjccXVnGzytbpFS53x4HM',
  'PayNote/Settlement Amount Specified':
    '4pVAdZo93FHRRkAkshqCZW4pUvvV1ccczJZ2Lu4jkD1D',
  'PayNote/Start Card Transaction Monitoring Requested':
    '649PKUqAakwi2JwDwHyypfWb4VmrrBAeEqzSU6FDWfgm',
  'PayNote/Transaction Details Update Failed':
    '2dmAPySotyaWEPRLBYMiooqrqa3v42pyrHYvWskMMsMb',
  'PayNote/Transaction Details Update Lock Change Failed':
    '1j22qLMeLkRgtwiMHWXJNopKeBHdACS4vELfLxc1mnA',
  'PayNote/Transaction Details Update Lock Requested':
    'DFFyom6z2nbWmFyoi3DLngRJKgLXXF4inq7hmZX7eShJ',
  'PayNote/Transaction Details Update Locked':
    'eoi7pHyfDPfN9EkHcCNMekiZubrBa9EyywmnWrGUsGb',
  'PayNote/Transaction Details Update Rejected':
    'tF2fYwf8b7HwErSWAduxWXoV8v7pMQPHVCsSZhZzhmT',
  'PayNote/Transaction Details Update Requested':
    '8tAYAsJki2mJb2qRu2H6dEYT2e3qQDqCFx2ugqtirLPU',
  'PayNote/Transaction Details Update Unlock Requested':
    '9xTYCggegp9Ci5AGfDk9avuKax1FC5EvwADrXtpLSebR',
  'PayNote/Transaction Details Update Unlocked':
    '8H1L8VGE9vXzbGgrv5RNkpYXngf43futPHQvnJ4SJD2B',
  'PayNote/Transaction Details Updated':
    'EsyQj8xWb1Kf2ESjDq1UR6PK2hx4fpnyGiEWJ14ttC8a',
  'PayNote/Transaction Identification Failed':
    'GvFQ71MZS1CrHvSopnTkSz8DBX2is4meMR5tpF5WmwCC',
  'PayNote/Transaction Identified':
    '6gQWKeJHZEbBo13Vvyf2nsdg7TA9kLSYdzMaJgySPG7V',
  'PayNote/Transaction Initiated':
    '14UHCXtf9XLpi3Z3n4xbo1dmXRzfXnDEH23iVaechxzh',
  'PayNote/Transaction Initiation Failed':
    'FSfMCJpFDcB9zNfrd4gDhaDXpFhaSNToGbAdxSkw55V7',
  'PayNote/Transaction Status': 'FutWoAEUEb3wkmEm4YP4JGFPtu3B3PQ3m71BmnRFFnsQ',
} as const;
export default blueIds;
