const meta = {
  "name": "PayNote",
  "aliases": {
    "PayNote/Capture Declined": "FzJqLm7Ek58LsVstJV2c37JfiMhRiWtjZNsiMz2ZfCYG",
    "PayNote/Capture Failed": "FUh3TARSh4TjnWKAkM5ydjFWLWEmrFByKMBQzcgQfqRW",
    "PayNote/Capture Funds Requested": "DvxKVEFsDmgA1hcBDfh7t42NgTRLaxXjCrB48DufP3i3",
    "PayNote/Child PayNote Issuance Declined": "DFKVw43E36kimqj64FyiiVxE9yNuB22SETFx5M4WAi9m",
    "PayNote/Child PayNote Issued": "FAZCx2s5eq9zPV64LdHNFYbjjxD3ci1ZqyTcQk5WhXAs",
    "PayNote/Funds Captured": "BJvjorbC5ed5KTV7SxoV3CvrJXjrFPcFxY9QT4jHBbXi",
    "PayNote/Funds Reserved": "AopfdGqnwcxsw4mJzXbmjDMnASRtkce9BZB1n6QSRNXX",
    "PayNote/Issue Child PayNote Requested": "53Dir2sGy1NHuCQXF6suGoDMxYacNhbcy23AKD89SghD",
    "PayNote/Payee Assignment Requested": "BVLMcTzag3D2rvg8LoKQ3Htgoqsh77EAoiTLTxH5aVBE",
    "PayNote/PayNote": "CDMVLRyodD2WhScu2PPRgGquEArMNGXxvZCYiJXg2YjT",
    "PayNote/PayNote Approved": "HQTUxErobqhSuhWo9DAC1WwaG9oYdjfmdKprGtV4TeEK",
    "PayNote/PayNote Cancellation Rejected": "GaYDPA7TTqWuoxioCYFPeyqomjH4g3YDtFxHv9yLRQ8A",
    "PayNote/PayNote Cancellation Requested": "DqiwzsNLbHCh6PaDF6wy6ZqBSF5JV5nAQSKFKTPRTbGB",
    "PayNote/PayNote Cancelled": "96buyUXwhkak8xKoCR5nAW9tMuwzkevJFdELVvwKxR6Y",
    "PayNote/PayNote Rejected": "AdKfkwRfzRUxUKSzhRfYANsaUBNnz4u6JFWR66qhzyZe",
    "PayNote/Reservation Declined": "4xS8bmZQBGPENmaPfsrtYguYfq4hTtaZAXrefdyFNkKq",
    "PayNote/Reservation Release Declined": "653sCbbRH3RiKhGjmVxh6wFVs4rn54wJRKDXRMKBZtjA",
    "PayNote/Reservation Release Requested": "GU8nkSnUuMs6632rHQyBndRtjDcMB9ZSbgwkGYcfGt97",
    "PayNote/Reservation Released": "CFqiZigjKE5JatANkaAkWw2NbgvEmb2BVEVPf3ckUrWg",
    "PayNote/Reserve Funds and Capture Immediately Requested": "3XstDYFkqsUP5PdM6Z6mwspPzgdQMFtUpNyMsKPK2o6N",
    "PayNote/Reserve Funds Requested": "3Y3TYmSfZMmPYKmF5i3eR8YcVPNP5Sic2bZN8xRnvMWm",
    "PayNote/Settlement Amount Rejected": "3b3ePGPg5GzS6KYfqoDfgjWbjccXVnGzytbpFS53x4HM",
    "PayNote/Settlement Amount Specified": "4pVAdZo93FHRRkAkshqCZW4pUvvV1ccczJZ2Lu4jkD1D"
  },
  "typesMeta": {
    "FzJqLm7Ek58LsVstJV2c37JfiMhRiWtjZNsiMz2ZfCYG": {
      "status": "stable",
      "name": "Capture Declined",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "FzJqLm7Ek58LsVstJV2c37JfiMhRiWtjZNsiMz2ZfCYG",
          "attributesAdded": []
        }
      ]
    },
    "FUh3TARSh4TjnWKAkM5ydjFWLWEmrFByKMBQzcgQfqRW": {
      "status": "stable",
      "name": "Capture Failed",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "FUh3TARSh4TjnWKAkM5ydjFWLWEmrFByKMBQzcgQfqRW",
          "attributesAdded": []
        }
      ]
    },
    "DvxKVEFsDmgA1hcBDfh7t42NgTRLaxXjCrB48DufP3i3": {
      "status": "stable",
      "name": "Capture Funds Requested",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "DvxKVEFsDmgA1hcBDfh7t42NgTRLaxXjCrB48DufP3i3",
          "attributesAdded": []
        }
      ]
    },
    "DFKVw43E36kimqj64FyiiVxE9yNuB22SETFx5M4WAi9m": {
      "status": "stable",
      "name": "Child PayNote Issuance Declined",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "DFKVw43E36kimqj64FyiiVxE9yNuB22SETFx5M4WAi9m",
          "attributesAdded": []
        }
      ]
    },
    "FAZCx2s5eq9zPV64LdHNFYbjjxD3ci1ZqyTcQk5WhXAs": {
      "status": "stable",
      "name": "Child PayNote Issued",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "FAZCx2s5eq9zPV64LdHNFYbjjxD3ci1ZqyTcQk5WhXAs",
          "attributesAdded": []
        }
      ]
    },
    "BJvjorbC5ed5KTV7SxoV3CvrJXjrFPcFxY9QT4jHBbXi": {
      "status": "stable",
      "name": "Funds Captured",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "BJvjorbC5ed5KTV7SxoV3CvrJXjrFPcFxY9QT4jHBbXi",
          "attributesAdded": []
        }
      ]
    },
    "AopfdGqnwcxsw4mJzXbmjDMnASRtkce9BZB1n6QSRNXX": {
      "status": "stable",
      "name": "Funds Reserved",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "AopfdGqnwcxsw4mJzXbmjDMnASRtkce9BZB1n6QSRNXX",
          "attributesAdded": []
        }
      ]
    },
    "53Dir2sGy1NHuCQXF6suGoDMxYacNhbcy23AKD89SghD": {
      "status": "stable",
      "name": "Issue Child PayNote Requested",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "53Dir2sGy1NHuCQXF6suGoDMxYacNhbcy23AKD89SghD",
          "attributesAdded": []
        }
      ]
    },
    "BVLMcTzag3D2rvg8LoKQ3Htgoqsh77EAoiTLTxH5aVBE": {
      "status": "stable",
      "name": "Payee Assignment Requested",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "BVLMcTzag3D2rvg8LoKQ3Htgoqsh77EAoiTLTxH5aVBE",
          "attributesAdded": []
        }
      ]
    },
    "CDMVLRyodD2WhScu2PPRgGquEArMNGXxvZCYiJXg2YjT": {
      "status": "stable",
      "name": "PayNote",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "CDMVLRyodD2WhScu2PPRgGquEArMNGXxvZCYiJXg2YjT",
          "attributesAdded": []
        }
      ]
    },
    "HQTUxErobqhSuhWo9DAC1WwaG9oYdjfmdKprGtV4TeEK": {
      "status": "stable",
      "name": "PayNote Approved",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "HQTUxErobqhSuhWo9DAC1WwaG9oYdjfmdKprGtV4TeEK",
          "attributesAdded": []
        }
      ]
    },
    "GaYDPA7TTqWuoxioCYFPeyqomjH4g3YDtFxHv9yLRQ8A": {
      "status": "stable",
      "name": "PayNote Cancellation Rejected",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "GaYDPA7TTqWuoxioCYFPeyqomjH4g3YDtFxHv9yLRQ8A",
          "attributesAdded": []
        }
      ]
    },
    "DqiwzsNLbHCh6PaDF6wy6ZqBSF5JV5nAQSKFKTPRTbGB": {
      "status": "stable",
      "name": "PayNote Cancellation Requested",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "DqiwzsNLbHCh6PaDF6wy6ZqBSF5JV5nAQSKFKTPRTbGB",
          "attributesAdded": []
        }
      ]
    },
    "96buyUXwhkak8xKoCR5nAW9tMuwzkevJFdELVvwKxR6Y": {
      "status": "stable",
      "name": "PayNote Cancelled",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "96buyUXwhkak8xKoCR5nAW9tMuwzkevJFdELVvwKxR6Y",
          "attributesAdded": []
        }
      ]
    },
    "AdKfkwRfzRUxUKSzhRfYANsaUBNnz4u6JFWR66qhzyZe": {
      "status": "stable",
      "name": "PayNote Rejected",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "AdKfkwRfzRUxUKSzhRfYANsaUBNnz4u6JFWR66qhzyZe",
          "attributesAdded": []
        }
      ]
    },
    "4xS8bmZQBGPENmaPfsrtYguYfq4hTtaZAXrefdyFNkKq": {
      "status": "stable",
      "name": "Reservation Declined",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "4xS8bmZQBGPENmaPfsrtYguYfq4hTtaZAXrefdyFNkKq",
          "attributesAdded": []
        }
      ]
    },
    "653sCbbRH3RiKhGjmVxh6wFVs4rn54wJRKDXRMKBZtjA": {
      "status": "stable",
      "name": "Reservation Release Declined",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "653sCbbRH3RiKhGjmVxh6wFVs4rn54wJRKDXRMKBZtjA",
          "attributesAdded": []
        }
      ]
    },
    "GU8nkSnUuMs6632rHQyBndRtjDcMB9ZSbgwkGYcfGt97": {
      "status": "stable",
      "name": "Reservation Release Requested",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "GU8nkSnUuMs6632rHQyBndRtjDcMB9ZSbgwkGYcfGt97",
          "attributesAdded": []
        }
      ]
    },
    "CFqiZigjKE5JatANkaAkWw2NbgvEmb2BVEVPf3ckUrWg": {
      "status": "stable",
      "name": "Reservation Released",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "CFqiZigjKE5JatANkaAkWw2NbgvEmb2BVEVPf3ckUrWg",
          "attributesAdded": []
        }
      ]
    },
    "3XstDYFkqsUP5PdM6Z6mwspPzgdQMFtUpNyMsKPK2o6N": {
      "status": "stable",
      "name": "Reserve Funds and Capture Immediately Requested",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "3XstDYFkqsUP5PdM6Z6mwspPzgdQMFtUpNyMsKPK2o6N",
          "attributesAdded": []
        }
      ]
    },
    "3Y3TYmSfZMmPYKmF5i3eR8YcVPNP5Sic2bZN8xRnvMWm": {
      "status": "stable",
      "name": "Reserve Funds Requested",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "3Y3TYmSfZMmPYKmF5i3eR8YcVPNP5Sic2bZN8xRnvMWm",
          "attributesAdded": []
        }
      ]
    },
    "3b3ePGPg5GzS6KYfqoDfgjWbjccXVnGzytbpFS53x4HM": {
      "status": "stable",
      "name": "Settlement Amount Rejected",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "3b3ePGPg5GzS6KYfqoDfgjWbjccXVnGzytbpFS53x4HM",
          "attributesAdded": []
        }
      ]
    },
    "4pVAdZo93FHRRkAkshqCZW4pUvvV1ccczJZ2Lu4jkD1D": {
      "status": "stable",
      "name": "Settlement Amount Specified",
      "versions": [
        {
          "repositoryVersionIndex": 0,
          "typeBlueId": "4pVAdZo93FHRRkAkshqCZW4pUvvV1ccczJZ2Lu4jkD1D",
          "attributesAdded": []
        }
      ]
    }
  }
} as const;
export default meta;
